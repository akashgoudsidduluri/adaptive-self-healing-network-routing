...
function transportLabInspector(){
  const source=selected?.kind==='device'?selected.id:(state.transport?.flows?.[0]?.source||state.devices?.[0]?.id||'');
  ...
  const destinationOptions=(state.devices||[]).filter(device=>device.id!==source).map(device=>`<option value="${esc(device.id)}">${esc(device.name)}</option>`).join('');
  ...
  const stateOrder=['CLOSED','SYN_SENT','SYN_RECEIVED','ESTABLISHED','FIN_WAIT','CLOSED'];
  const currentState=active?.state||'CLOSED';
  const comparison=state.transport?.result;
  const comparisonHtml=comparison?.results?.length?`<table class="diagnostic-table"><thead><tr><th>Protocol</th><th>Latency</th><th>Loss</th><th>Retrans.</th></tr></thead><tbody>${comparison.results.map(row=>`<tr><td>${esc(row.protocol)}</td><td>${fmt((row.average_latency||0)*1000,1)} ms</td><td>${fmt(row.packets_lost||0,0)}</td><td>${fmt(row.retransmissions||0,0)}</td></tr>`).join('')}</tbody></table>`:'';
  return `<div class="device-hero"><div class="device-symbol">⇄</div><div><b>Transport Lab</b><span>TCP / UDP over the live topology</span></div></div><div class="inspector-section"><div class="section-title">Transport setup</div><label>Protocol<select id="transportProtocol"><option ${protocol==='TCP'?'selected':''}>TCP</option><option ${protocol==='UDP'?'selected':''}>UDP</option></select></label><label>Source<select id="transportSource">${sourceOptions}</select></label><label>Destination<select id="transportDestination">${destinationOptions}</select></label><div class="two"><label>Source port<input id="transportSourcePort" type="number" value="${active?.source_port||5000}"/></label><label>Destination port<input id="transportDestinationPort" type="number" value="${active?.destination_port||8080}"/></label></div><label>Payload (KB)<input id="transportPayload" type="number" min="1" value="${Math.max(1,Math.round((active?.packets?.find(p=>p.kind==='DATA')?.payload_size||1000)/1024))}"/></label><div id="tcpTransportSettings" class="${protocol==='UDP'?'hidden':''}"><div class="two"><label>Initial CWND<input id="transportCwnd" type="number" min="1" value="${active?.initial_cwnd||1}"/></label><label>Receiver window<input id="transportReceiverWindow" type="number" min="1" value="${active?.receiver_window||8}"/></label></div><div class="two"><label>SSTHRESH<input id="transportSsthresh" type="number" min="1" value="${active?.ssthresh||16}"/></label><label>Timeout (ms)<input id="transportTimeout" type="number" min="1" value="${Math.round((active?.timeout||.1)*1000)}"/></label></div></div><div class="button-row"><button data-transport-action="create">${active||udp.length?'Start Connection':'Start Flow'}</button><button data-transport-action="send">Send Data</button><button data-transport-action="close">Close</button><button data-transport-action="tick">Tick</button><button data-transport-action="reset">Reset</button></div></div><div class="inspector-section"><div class="section-title">TCP state</div><div class="tcp-state-machine">${stateOrder.map(item=>`<span class="tcp-state ${item===currentState?'current':''}">${item.replace('_',' ')}</span>`).join('<i>→</i>')}</div></div><div class="inspector-section"><div class="section-title">Live ${esc(protocol)} metrics</div><div class="diagnostic-stats">${[['Packets',protocolStats.packets_sent||0],['Delivered',protocolStats.packets_delivered||0],['Loss',`${fmt(protocolStats.packets_lost||0,0)}`],['Throughput',`${fmt(protocolStats.throughput||0,1)} B/s`],['RTT',active?.average_rtt_ms==null?'—':`${fmt(active.average_rtt_ms,1)} ms`],['CWND',active?.cwnd==null?'—':fmt(active.cwnd,1)],['SSTHRESH',active?.ssthresh==null?'—':fmt(active.ssthresh,1)],['In flight',active?.packets_in_flight??0],['ACKs',active?.ack_count??0],['Retransmissions',active?.retransmission_count??0],['Timeouts',active?.timeout_count??0]].map(([label,value])=>`<div class="diagnostic-stat"><b>${esc(value)}</b><span>${label}</span></div>`).join('')}</div></div><div class="inspector-section"><div class="section-title">Active transport flows</div>${(connections.length+udp.length)?[...connections,...udp].map(flow=>`<div class="interface-card"><b>${esc(flow.flow_id)} · ${esc(flow.source)}:${flow.source_port} → ${esc(flow.destination)}:${flow.destination_port}</b><div>${esc(flow.state||'UDP')} · ${esc(flow.traffic_class)}</div></div>`).join(''):'<p class="hint">No transport flow.</p>'}</div><div class="inspector-section"><div class="section-title">TCP vs UDP comparison</div><button data-transport-action="compare">Run deterministic comparison</button>${comparisonHtml}<div class="hint">Uses the same topology, conditions, workload, and seed.</div></div>`;
}
function bindTransportLab(){
  inspectorBody.querySelectorAll('[data-transport-action]').forEach(button=>button.addEventListener('click',async()=>{
    ...
    try{
      ...
      if(action==='send'){const flow=state.transport?.flows?.find(item=>item.protocol===protocol||(!item.state&&protocol==='UDP'));if(!flow)throw new Error('Start a transport flow first.');await api('transport_send',{protocol,flow_id:flow.flow_id,packet_count:1,payload_size:payload});}
  ...
  $('transportProtocol')?.addEventListener('change',()=>{transportProtocol=$('transportProtocol').value;const tcp=$('tcpTransportSettings');if(tcp)tcp.classList.toggle('hidden',transportProtocol==='UDP');});
